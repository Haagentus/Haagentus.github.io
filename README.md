...
- `index.html`：站点首页沿用原有全站背景图片，叠加浅银蓝月相环形加载徽记与百分比读数，色调与背景的蓝紫月光呼应；徽记直接引用用户参考图的原始图案，并将黑底转为透明以融入背景，加载展示约 6 秒；页面就绪后显示带星芒装饰线的“进入”，点击首屏任意位置可跳转导航页。下方保留关于、联系与资源入口。首页样式与加载交互分别位于 `css/home-lunar.css` 和 `js/home-lunar.js`，支持移动端、减少动态效果偏好与无脚本访问。
- `css/design-system.css`：全站共享视觉层，统一色彩、字体、圆角、面板、控件、卡片、交互状态和移动端间距。
- `404.html`：404错误页面，当访问不存在的页面时显示，提供返回首页、返回上一页和浏览导航的链接。
- 共享导航栏提供中英文切换按钮；切换到英文时，会从字体清单中随机选择一个自创文字字体渲染英文文案。
- `navigation/`：卡片式网站导航，收录包括 Genshin Impact Wiki、Teyvat WeChat 在内的米哈游相关站点，支持搜索和分类筛选。
